...
tmp_act, s_flag, before_dnum = None, False, {'RIGHT': 0, 'LEFT': 0}
engine = pyttsx3.init()
dial_flag = False
...
def count_hand_finger(left_hand, right_hand):
    # 손가락이 접힘 = true
    ...
    hand_landmarks_index = [
        [mp_holistic.HandLandmark.THUMB_TIP, mp_holistic.HandLandmark.THUMB_IP, mp_holistic.HandLandmark.THUMB_MCP],
        [mp_holistic.HandLandmark.THUMB_IP, mp_holistic.HandLandmark.THUMB_MCP, mp_holistic.HandLandmark.THUMB_CMC],
        [mp_holistic.HandLandmark.INDEX_FINGER_DIP, mp_holistic.HandLandmark.INDEX_FINGER_PIP,
         mp_holistic.HandLandmark.INDEX_FINGER_MCP],
        [mp_holistic.HandLandmark.MIDDLE_FINGER_DIP, mp_holistic.HandLandmark.MIDDLE_FINGER_PIP,
         mp_holistic.HandLandmark.MIDDLE_FINGER_MCP],
        [mp_holistic.HandLandmark.RING_FINGER_DIP, mp_holistic.HandLandmark.RING_FINGER_PIP,
         mp_holistic.HandLandmark.RING_FINGER_MCP],
        [mp_holistic.HandLandmark.PINKY_DIP, mp_holistic.HandLandmark.PINKY_PIP, mp_holistic.HandLandmark.PINKY_MCP]]
    ...
    for hand_index in range(c):
        ...
        for num, i in enumerate(hand_landmarks_index):
            ...
            if (num != 0 and num != 1 and num != 5):
                if angle < 110:
                    fingers_statuses[hand_label + "_" + finger_name[num]] = True
                    hands_count[hand_label] -= 1
            # 엄지, 소지
            else:
                if angle < 150:
                    # 엄지가 2번 카운트 되는 것을 방지하기 위한 if문
                    if (fingers_statuses[hand_label + "_" + finger_name[num]] == False):
                        fingers_statuses[hand_label + "_" + finger_name[num]] = True
                        hands_count[hand_label] -= 1
    return hands_count, fingers_statuses


def hands_static_gesture(results, left_hand, right_hand, finger_statuses, hands_counts):
    gesture = {'LEFT': "", 'RIGHT': ""}
    ...
    for hand_index in range(c):
        ...
        min_len1 = xy_len(hand_landmarks.landmark[mp_holistic.HandLandmark.MIDDLE_FINGER_TIP],
                          hand_landmarks.landmark[mp_holistic.HandLandmark.MIDDLE_FINGER_DIP]) // 2 * 3
        min_len2 = xy_len(hand_landmarks.landmark[mp_holistic.HandLandmark.MIDDLE_FINGER_PIP],
                          hand_landmarks.landmark[mp_holistic.HandLandmark.MIDDLE_FINGER_MCP])
        pen_min_len = xy_len(hand_landmarks.landmark[mp_holistic.HandLandmark.WRIST],
                             hand_landmarks.landmark[mp_holistic.HandLandmark.MIDDLE_FINGER_MCP]) // 4
        ...
        if pen_mode[hand_label.upper()] != "off":
            pen_point[hand_label.upper()] = hand_landmarks.landmark[mp_holistic.HandLandmark.THUMB_TIP]
        # 정적 제스처 파악
        ...
        if hands_counts[hand_label.upper()] == 2:
            if (finger_statuses[hand_label.upper() + "_INDEX"] == False) and (
                ...
                # 중지와 검지가 붙어있으면
                if xy_len(hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_TIP],
                          hand_landmarks.landmark[mp_holistic.HandLandmark.MIDDLE_FINGER_TIP]) < min_len1:
        ...
        #         if xy_len(hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_TIP],
        #                   hand_landmarks.landmark[mp_holistic.HandLandmark.MIDDLE_FINGER_TIP]) < min_len2:
        #             gesture[hand_label.upper()] = "dial"
        #             update_degree(hand_label, hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_TIP],
        ...
        # 손가락 숫자가 3~5 -> 펜 모드, dial 확인
            ...
            if xy_len(hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_TIP],
                ...
                dial_none_count[hand_label.upper()] = 0
        # pen 제스처
        elif xy_len(hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_TIP],
                    hand_landmarks.landmark[mp_holistic.HandLandmark.THUMB_TIP]) < pen_min_len:
            pen_gesture_count[hand_label.upper()] += 1

            gesture[hand_label.upper()] = "pen"
        # pen off 제스처
        elif xy_len(hand_landmarks.landmark[mp_holistic.HandLandmark.MIDDLE_FINGER_TIP],
                    hand_landmarks.landmark[mp_holistic.HandLandmark.THUMB_TIP]) < min_len1:
...
def hands_dynamic_gesture(results, left_hand, right_hand):
    global dial_flag
    action = {'LEFT': "", 'RIGHT': ""}
    # for hand_index, hand_info in enumerate(results.multi_handedness):
    #     hand_label = hand_info.classification[0].label
    ...
    for hand_index in range(c):
        if c > 1:
            if hand_index == 0:
                ...
                hand_label = "RIGHT"
                hand_landmarks = left_hand
        point_min_len = xy_len(hand_landmarks.landmark[mp_holistic.HandLandmark.WRIST],
                               hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_MCP])
...
def mouse_point(hand_label, hand_landmarks):
    point1_x = hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_MCP].x
    point1_y = hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_MCP].y
    point2_x = hand_landmarks.landmark[mp_holistic.HandLandmark.MIDDLE_FINGER_MCP].x
    point2_y = hand_landmarks.landmark[mp_holistic.HandLandmark.MIDDLE_FINGER_MCP].y
    point3_x = (point1_x + point2_x) / 2
    point3_y = (point1_y + point2_y) / 2
    point_x = (2 * point3_x - hand_landmarks.landmark[mp_holistic.HandLandmark.WRIST].x) * width
    point_y = (2 * point3_y - hand_landmarks.landmark[mp_holistic.HandLandmark.WRIST].y) * height
    if hand_label == "LEFT":
...
def update_point(hand_label, hand_landmarks):
    point_x = int(hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_TIP].x * width)
    point_y = int(hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_TIP].y * height)
    if hand_label == "LEFT":
...
def act_sound(speak, gesture, func = None):
    a = ""
    #print("speak",speak)
    #print("func",func)
    #print("test", speak.items(), gesture)
    if not isinstance(func, type(None)):
        func = ",".join(func.split("//"))
        #print("split_func",func)
        a = func
    else:
        for (key_, val_),(key, val) in zip(speak.items(), gesture.items()):
            #print("key",key,"val",val,"key_",key_, "val_",val_)
            if key == "LEFT" or key_ == "LEFT":
                a += (val + " ")  # "왼쪽 손"
                a += val_
                #print("최종", a)
    ...
    try:
        #print(a)
        #print("act_sound", speak)
        engine.say(a)
        engine.runAndWait()
    except:
        print("동시에 실행할 수 없습니다")
        pass

# 화면에 손 관련 그림
def hand_drawing(image, results, gesture, action, left_hand, right_hand):
    global tmp_act, s_flag, before_dnum, dial_flag
    #print("dial_num", dial_num.values(), before_dnum.values())
    # for hand_index, hand_info in enumerate(results.multi_handedness):
    #     hand_label = hand_info.classification[0].label
    #     hand_landmarks = results.multi_hand_landmarks[hand_index]
    c = 0
    ...
    for hand_index in range(c):
        ...
        if action_last[hand_label.upper()] == tmp_act:
            s_flag = False
        elif tmp_act != None and action_last[hand_label.upper()] != None:
            s_flag = True
        elif dial_flag == True and gesture[hand_label.upper()] == "dial":
            s_flag = True



        #print("~!!!!!!!!!!~",action_last[hand_label.upper()], dial_flag, tmp_act)
        #print(action_last, gesture, s_flag)
        # if s_flag == True:
        #     print(s_flag, "True로 바뀌었습니다")
        #     s = Thread(target=act_sound, args=(action_last,))
        #     s.start()
        ...
        if action_last[hand_label.upper()] != "none":
            ...
            print("action_last none임.",action_last[hand_label.upper()])
        ...
        if (gesture[hand_label.upper()] == "mouse") or ("click" in gesture[hand_label.upper()]):
            ...
            image = cv2.circle(image, (int(hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_TIP].x * width),
                                       int(hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_TIP].y * height)),
                               1, (20, 255, 155), 5)
            ...
            image = cv2.putText(image, temp, (10, 60), cv2.FONT_HERSHEY_SIMPLEX,
                                1, (20, 255, 155), 3, 5)
            image = cv2.line(image, (int(hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_TIP].x * width),
                                     int(hand_landmarks.landmark[mp_holistic.HandLandmark.INDEX_FINGER_TIP].y * height)), (
                                 int(hand_landmarks.landmark[mp_holistic.HandLandmark.THUMB_TIP].x * width),
                                 int(hand_landmarks.landmark[mp_holistic.HandLandmark.THUMB_TIP].y * height)), (255, 0, 0),
                             5)
        ...
        if s_flag == True:
            print(s_flag, "True로 바뀌었습니다")
            speak = action_last
            if gesture[hand_label.upper()] != "dial":
                temp = None
            # s = Thread(target=act_sound, args=(action_last, temp))
            s = Thread(target=act_sound, args=(speak, gesture, temp))
            s.start()
        tmp_act = action_last[hand_label.upper()]
        before_dnum = dial_num
