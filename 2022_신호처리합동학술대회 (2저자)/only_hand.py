...
# menu
# menu_sta : off, light, radio, tv, air, exit
menu_sta = {"main": 'off', "sub": "off"}
main_list = ["light", "radio", "tv", "air", "exit"]
main_num = 0
light_level = 3
radio_sta = {"volume" : 0, "channel" : 0}
...
# 화면에 손 관련 그림
def hand_drawing(image, results, gesture):
    for hand_index, hand_info in enumerate(results.multi_handedness):
        hand_label = hand_info.classification[0].label
        hand_landmarks = results.multi_hand_landmarks[hand_index]
        ...
        if (gesture[hand_label.upper()] == "mouse") or ("click" in gesture[hand_label.upper()]):
            ...
            for i in range(4):
                if dial_points_statuses[hand_label.upper()][i] == 0:
                    ...
                    color = (255, 0, 0)
                image = cv2.circle(image, (
                dial_next_points[hand_label.upper()][i][0], dial_next_points[hand_label.upper()][i][1]), 1, color, 5)
        ...
        if hand_action != "":
            ...
            # 메인 메뉴 on
                ...
                # 서브 메뉴 on
                    ...
                    # radio 서브 메뉴 조작
                    if menu_sta["main"] == "radio":
                        #오른손 -> 채널 조작
                        if hand == "RIGHT":
                            if hand_action == "plus":
                                radio_sta["channel"] += 1
                                if radio_sta["channel"] > 30:
                                    radio_sta["channel"] = 30
                            elif hand_action == "minus":
                                radio_sta["channel"] -= 1
                                if radio_sta["channel"] < 0:
                                    radio_sta["channel"] = 0
                        #왼손 -> 볼륨 조작
                        else:
                            if hand_action == "plus":
                                radio_sta["volume"] += 1
                                if radio_sta["volume"] > 30:
                                    radio_sta["volume"] = 30
                            elif hand_action == "minus":
                                radio_sta["volume"] -= 1
                                if radio_sta["volume"] < 0:
                                    radio_sta["volume"] = 0
                    # tv 서브 메뉴 조작
                    if menu_sta["main"] == "tv":
                        # 오른손 -> 채널 조작
    ...
    if menu_sta["main"] != "off":
        ...
        if menu_sta["sub"] == "on":
            if menu_sta["main"] == "light":
                ...
                text2 = "channel:" + str(radio_sta["channel"])
                text1 = "volume:" + str(radio_sta["volume"])
                image = cv2.putText(image, text1, (300, 130), cv2.FONT_HERSHEY_SIMPLEX,
                                    1, (0, 0, 0), 2, 5)
                image = cv2.putText(image, text2, (500, 130), cv2.FONT_HERSHEY_SIMPLEX,
                                    1, (0, 0, 0), 2, 5)
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            imgae = hand_drawing(image, results, gesture)
