...
MAX_FRAMES = 249 #(80000) #343 #(110000)
...
def preprocess_function(examples, input_column = "path", output_column = "emotion"):
    ...
    :param target_sampling_rate:[int] Global variable with the expected sampling rate of the model
    """
    speech_list = [data for data in examples["input_value__"]]
    ...
    print(np.array(result["input_values"]).shape)
    ###########
    # print(np.array(result["attention_mask"]).shape)
    # audio_lengths = tf.cumsum(result["attention_mask"], -1)[:, -1]
    # print(audio_lengths)
    # wav2vec2 = TFWav2Vec2Model.from_pretrained(
    #     MODEL_CHECKPOINT, apply_spec_augment=False, from_pt=True
    # )
    # hidden_states = wav2vec2(examples["input_value__"][0])[0]
    #tf.print("hidden_states: ", hidden_states[0].shape)
    # feature_lengths = wav2vec2.wav2vec2._get_feat_extract_output_lengths(
    #     audio_lengths
    # )
    # print(feature_lengths) #343
...
class TFWav2Vec2ForAudioClassification(layers.Layer):
    ...
    def call(self, inputs):
        ...
        #tf.print("hidden_states", hidden_states.shape)
        ...
        #         audio_lengths
        #     )
        #     #tf.print("audio_lengths", audio_lengths.shape, "feature_lengths", feature_lengths.shape)
        #     pooled_state = mean_pool(hidden_states, feature_lengths)
...
def build_model():
    ...
    x = layers.Dense(HIDDEN_DIM)(wav2vec2_model)
...
if __name__ == "__main__":
    ...
    with tf.device('/device:GPU:1'):
        ...
        for fold in range(5):  # 5-CV strategy
            ...
            #
            #
            # test_d = np.load("./new_data/aug_wav_test_data_2023-10-07_16-37-37.npy", allow_pickle=True).item()
            # train_d = np.load("./new_data/aug_wav_train_data_2023-10-07_16-37-37.npy", allow_pickle=True).item()
            # train_inputs, test_inputs = train_d["x"], test_d["x"]
            # #test_inputs = test_d["x"]
            # print("!!!!!!!!!!!", train_inputs.shape, test_inputs.shape)
            #
            # train_dataset = pd.DataFrame(train_dataset)
            # train_dataset["input_value__"] = list(train_inputs)
            # train_dataset = Dataset.from_pandas(train_dataset)
            # print(train_dataset)
            #
            # eval_dataset = pd.DataFrame(eval_dataset)
            # eval_dataset["input_value__"] = list(test_inputs)
            ...
            # eval_dataset = eval_dataset.remove_columns("input_values")
            # print("===remove columns====")
            # print(train_dataset)
            # print(eval_dataset)
            # train_dataset = train_dataset.rename_column("input_value__", "input_values")
            # eval_dataset = eval_dataset.rename_column("input_value__", "input_values")
            ...
            print("validation dataset", np.array(eval_dataset["input_values"]).shape, np.unique(np.array(eval_dataset["labels"])))
            ...
            weight_path=folder_address+'/10-fold_weights_best_'+str(fold)+f"_{{val_accuracy:.5f}}.hdf5"
